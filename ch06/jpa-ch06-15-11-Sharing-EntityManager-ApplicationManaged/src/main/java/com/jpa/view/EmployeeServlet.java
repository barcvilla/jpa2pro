 ...
 * 
 * Hay un peligro en mezclar multiples contextos de persistencia en la misma transaccion JTA. Esto ocurre cuando multiple
 * contexto de persistencia application-managed llega a estar sincronizado con la transaccion o cuando el contexto de
 * persistencia llega a estar mezclado con el contexto de persistencia container-managed. Cuando la transaccion commits,
 * cada contexto de persistencia recibira una notificacion desde el manager transaction que los cambios deberian ser
 * escrito en la BD. esto causara que  cada contexto de persistencia  sea igualado.
 * 
 * Que pasa si una entidad con la misma llave primaria es usada en mas de un contexto de persistencia? que version de
 * entidad sera almacenada? La respuesta desafortunada es que no hay forma de estar seguro. El contenedor no garantiza
 * ningun ordenamiento cuando se notifica al contexto de persistencia de la terminacion de la tramsaccion. como resultado
 * es critico para la integridad de los datos que las entidades que no nunca han sido por mas de un contexto de persistencia
 * en la misma transaccion. Cuando disenamos la aplicacion, recomendamos recoger una estrategia de un solo contexto de
 * persistencia (container-managed o application managed) y seguir con la estrategia consistentemente.
 * @author PC
 */
